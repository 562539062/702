...
# 采用频率一样，不再分别读取姿态和应力数据
    ...
    for loc in range(len(filename)):
        name = os.path.join(pathstr,filename[loc])
    ...
    for k in range(NM): # 信号个数
        ...
        if kk >= 2:
            ...
            for j in range(kk-1):
                ...
                Result[j][6]=np.max(np.array(sd[(inn[j]):(inn[j+1]+1)])) # 合成峰值
                Result[j][7]=np.min(np.array(sd[(inn[j]):(inn[j+1]+1)])) # 合成谷值
...
def fun_save(path,dtype,data):
    ...
    if dtype=='npy':
        np.save(path,data)
###################获取实际的每个文件需要跑的时间段数###################
def getfileseg(runfiles):
    sumS=[]
    for fn in range(len(runfiles)):
        if runfiles[fn]==runfiles[-1]:
            tempS=s.sumSegment
        else:
            tempS=math.floor((datetime.strptime(runfiles[fn+1],"%Y%m%d%H%M%S")-\
                datetime.strptime(runfiles[fn],"%Y%m%d%H%M%S"))/timedelta(minutes=30))
        tempS=min(s.sumSegment,tempS)
        sumS.append(tempS)
    return sumS
...
def createtimeline(runfiles):
    timeline=[]
    sumS=getfileseg(runfiles)
    for fn in range(len(runfiles)):
        file_hour=int(runfiles[fn][8:-4])
        file_min=int(runfiles[fn][10:-2])
        for Segment in range(1,sumS[fn]+1):
            timeline.append(runfiles[fn][0:4]+'-'+runfiles[fn][4:6]+'-'+runfiles[fn][6:8]+' '
                            +str((file_hour+int((30*(Segment-1)+file_min)/60))%24)+':'
                            +str((file_min+((Segment-1)%2)*30)%60))
    return timeline



#################用于将运算结果保存为excel，plot的数据##############################            
def pddeal(savepath,runfiles,result):   #由于多进程乱序运行，保存所得plot数据不按顺序保存，将分别保存的数据按顺序保存入一个文件中
    sumS=getfileseg(runfiles)
    for j in s.index_3:
        nowf=0
        for f in range(len(runfiles)):        
            for i in range(len(s.pointname)):
                path=savepath+'plot_data/'+s.Signame[j]+'/'+runfiles[f]+'/'+s.pointname[i]+'.npy'
                sumdata=[]
                for seg in range(sumS[f]):
                    sumdata.append(result[nowf+seg][j][i])
...
def signal1(path,ffilename,savepath):
    ...
    [Sig, Len, Nright, Nerror]= loadSignal20201129(s.filename, pathstr, segment_length = 720000, padding=False)
    ...
    if True:
        ...
        for loc in Nright:
            ...
            fname=savepath+'img/'+ffilename+'/{}.png'.format(s.filename[loc])
...
def signal2(args_list):
    path=args_list[0]
    ffilename=args_list[1]
    savepath=args_list[2]
    Segment=args_list[3]
    Settings=args_list[4]
    ...
    # 总通道数164个
    pathstr = os.path.join(path, ffilename)  # 文件路径
    [Sig, Len, Nright, Nerror] = loadSignal20201129(s.filename, pathstr, segment_length = 720000)
    ...
    if Segment<8:
        Index1=list(range(Settings.Slen*(Segment-1)+1,Settings.Slen*Segment+1))
        # Index2=2*90000*(Segment-1)+1:2:90000*Segment*2;
        DSig=np.zeros((Settings.Slen,s.DSig_shape1))
    else:
        Index1=list(range(Settings.Slen*(Segment-1)+1,Settings.Slen_+1))
        # Indexl2=2*90000*(Segment-1)+1:2:1439400;
        DSig=np.zeros((Settings.Slen,s.DSig_shape1)); # 为保持数据长度一致，最后一个文件减少3分钟采用数据

    for j in s.DSigRange: 
    ...
    # 对单向应力传感器进行6分钟分段趋势滤除
    for j in range(Settings.forcenum):
        DSig[:,j]=Sig_detrend(DSig[:,j],Settings.SFreq) # 消除零飘
        if False:
            plt.clf()
            plt.plot(DSig[:,j])
            plt.title(Signame[j])
    ...
    # fun_save(fname,'mat',{'DSig':DSig})
    [m,n]=np.shape(DSig)
    temp=fftfilter(DSig,Settings.SFreq,Settings.F2,Settings.F4); #	提取波浪和高频合成部分
    Result_Sample=De_Sample(temp,Settings.SFreq,Settings.F2,Settings.F3); # 提取子样
    # 分析锚链张力信号
    ...
    for j in s.index_2:
        Zk=Zk+1
        Z21=fftfilter(DSig[:,j],Settings.SFreq,Settings.F1,Settings.F3) # 提取分析信号，0.01-0.5Hz
        # Result_Sample(j)=De_Sample(temp1,Settings.SFreq,F1,Settings.F2); # 提取子样
        Z22=fftfilter(Z21,Settings.SFreq,Settings.F2,Settings.F3); # 无慢飘信号
