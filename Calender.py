...
class Calendar:
    ...
    def __init__(s, point = None,fileyear=datetime.now().year,filemonth=datetime.now().month, position = None):
    ...
    def __place_widgets(s):
        ...
        s.CB_year = ttk.Combobox(hframe, width = 5, values = [str(year) for year in range(s.fileyear, s.fileyear-11,-1)], validate = 'key', validatecommand = (Input_judgment_num, '%P'))
        s.CB_year.current(0)
        ...
        s.CB_month.current(s.filemonth - 1)
